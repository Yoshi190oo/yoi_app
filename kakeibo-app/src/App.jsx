...
// ─── Default Data ──
const DEFAULT_CATS = [
  {id:1, name:"家賃・光熱費", icon:"home",     color:"#3B82F6", monthBudget:950, dayBudget:null, split:"need"},
  {id:2, name:"食料品",       icon:"shopping", color:"#F59E0B", monthBudget:320, dayBudget:12,   split:"need"},
  {id:3, name:"交通費",       icon:"car",      color:"#8B5CF6", monthBudget:90,  dayBudget:null, split:"need"},
  {id:4, name:"外食",         icon:"utensils", color:"#EF4444", monthBudget:120, dayBudget:8,    split:"want"},
  {id:5, name:"医療・健康",   icon:"shield",   color:"#10B981", monthBudget:60,  dayBudget:null, split:"need"},
  {id:6, name:"娯楽・文化",   icon:"music",    color:"#6366F1", monthBudget:100, dayBudget:null, split:"want"},
  {id:7, name:"衣類",         icon:"tag",      color:"#F97316", monthBudget:80,  dayBudget:null, split:"want"},
  {id:8, name:"その他",       icon:"box",      color:"#6B7280", monthBudget:150, dayBudget:null, split:"want"},
...
const DEFAULT_SUBS = [
  {id:"s1", name:"Netflix",     amount:15.99, cycle:"monthly", note:"動画",     color:"#E50914", split:"want"},
  {id:"s2", name:"Spotify",     amount:9.99,  cycle:"monthly", note:"音楽",     color:"#1DB954", split:"want"},
  {id:"s3", name:"iCloud+",     amount:2.99,  cycle:"monthly", note:"50 GB",    color:"#3B82F6", split:"need"},
  {id:"s4", name:"Adobe CC",    amount:54.99, cycle:"monthly", note:"制作",     color:"#FF0000", split:"want"},
];
...
const SEED = [
  {id:100,type:"expense",catId:1, desc:"家賃",     amount:950,  date:cy+"-"+cm+"-01", accountId:null},
  {id:102,type:"expense",catId:3, desc:"定期券",   amount:89,   date:cy+"-"+cm+"-01", accountId:null},
  {id:106,type:"income", catId:"i1",desc:"給与",   amount:3200, date:cy+"-"+cm+"-01", accountId:null},
  {id:107,type:"income", catId:"i2",desc:"副業",   amount:480,  date:cy+"-"+cm+"-12", accountId:null},
  {id:101,type:"expense",catId:2, desc:"スーパー", amount:74.5, date:sd(6), accountId:null},
  {id:103,type:"expense",catId:4, desc:"カフェ",   amount:18.5, date:sd(5), accountId:null},
  {id:104,type:"expense",catId:2, desc:"八百屋",   amount:42.3, date:sd(4), accountId:null},
  {id:105,type:"expense",catId:6, desc:"映画館",   amount:24,   date:sd(3), accountId:null},
  {id:108,type:"expense",catId:4, desc:"外食",     amount:32,   date:sd(2), accountId:null},
  {id:109,type:"expense",catId:8, desc:"Amazon",   amount:29.99,date:sd(1), accountId:null},
  {id:110,type:"expense",catId:2, desc:"コンビニ", amount:55.8, date:TODAY_STR, accountId:null},
  {id:111,type:"expense",catId:4, desc:"ランチ",   amount:14.5, date:TODAY_STR, accountId:null},
];
...
const STORAGE_KEY = "haushaltsbuch_data_v1";
...
export default function App(){
  ...
  function buildInitialSavingsHistory(){
    ...
    for(var i=5; i>=0; i--){
      ...
      base += 600 + Math.round(Math.random()*200);
      hist.push({year:d.getFullYear(), month:d.getMonth(), amount:base});
    }
    return hist;
  }
      ...
      {tab===3 && (
        <div>
          ...
          <div style={{padding:"0 18px"}}>
            ...
            <CollapsibleSection iconName="piggy" title="初期所持金・残高設定" subtitle="アプリ開始時の残高を一括入力">
              <div style={{padding:"14px 16px"}}>
                <div style={{fontSize:12,color:C.textMid,marginBottom:14,lineHeight:1.6}}>
                  アプリを初めて使う際に、各資産の現在の残高を一括で入力できます。入力した金額は各資産の残高に直接反映されます。
                </div>
                {accounts.map(function(acc){
                  return (
                    <div key={acc.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderTop:"1px solid "+C.border}}>
                      <div style={{width:34,height:34,borderRadius:10,background:acc.color+"18",border:"1.5px solid "+acc.color+"30",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                        <Icon name={acc.isSavings?"piggy":"bank"} size={16} color={acc.color}/>
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:13,fontWeight:600,color:C.text}}>{acc.name}</div>
                        <div style={{fontSize:10,color:C.textMuted}}>{acc.bank}</div>
                      </div>
                      ...
                      }} style={{width:110,background:C.surfaceAlt,border:"1.5px solid "+C.border,color:C.text,fontFamily:"inherit",outline:"none",borderRadius:8,padding:"8px 10px",fontSize:13,textAlign:"right",fontWeight:700}}/>
                ...
                <button onClick={function(){alert("初期残高が保存されました");}} style={{width:"100%",marginTop:10,padding:"12px",borderRadius:12,border:"none",background:C.blue,color:"#fff",fontSize:13,fontWeight:700,fontFamily:"inherit",cursor:"pointer"}}>保存</button>
              </div>
            </CollapsibleSection>
